...
export interface DeepCopy {
    <V>(value:V,typeCopyers?:TypeRevivers<Copier>|null,maxDepth?:number):V;
 ...
 */
function deepCopyByRecursive<V,Key,Host>(value:V,typeReviverObject:TypeReviverObject<Copier>,maxDepth:number,startDepth:number,rawCopyMap:Map<any,any>,key?:Key,host?:Host):V {
    ...
    function copyMember<T,K,H>(member:T,key?:K,host:H|V = value,memberDepth?:number):T {
        ...
        return deepCopyByRecursive(member,typeReviverObject,maxDepth,memDepth,rawCopyMap,key,host);
    }
    ...
    if (typeof copier === "function"){
        const itemCopy = copier.call(value,value,copyMember,key,host,typeName,startDepth);
        rawCopyMap.set(value,itemCopy);
        return  itemCopy;
    }

    if (typeof (<any>value).getCopy === "function"){
        const itemCopy = (<any>value).getCopy(value,copyMember,key,host,typeName,startDepth)
        rawCopyMap.set(value,itemCopy);
        return itemCopy;
    }
    ...
    if (typeof (<any>value)[Symbol.iterator] === "function"){
        ...
        for (const item of value){
            const itemCopy = deepCopyByRecursive(item,typeReviverObject,maxDepth,nextDepth,rawCopyMap,index,value);
    ...
    return  allKeys.reduce(function (obj,key) {
        ...
        const itemCopy = deepCopyByRecursive(propVal,typeReviverObject,maxDepth,nextDepth,rawCopyMap,key,value);
        obj[key] = itemCopy
...
export function createDeepCopy(presetTypeCopierMap?:TypeReviverMap<Copier>):DeepCopy {
    ...
    function deepCopy<V,Key,Host>(value:V,typeCopyers?:TypeRevivers<Copier>|null,maxDepth?:number):V {
        ...
        return deepCopyByRecursive(value,trObj,maxDepth,0,new Map());
    }
