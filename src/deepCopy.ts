...
export interface CopyOptions<Key,Host> {
    key?:Key;
    host?:Host;
    startDepth?:number;
}
...
export interface DeepCopy {
    <V,Key,Host>(value:V,typeCopyers?:TypeRevivers<Copier>|null,maxDepth?:number,options?:CopyOptions<Key,Host>):V;
...
export function createDeepCopy(presetTypeCopierMap?:TypeReviverMap<Copier>):DeepCopy {
    ...
    function deepCopy<V,Key,Host>(value:V,typeCopyers?:TypeRevivers<Copier>|null,maxDepth?:number,options:CopyOptions<Key,Host> = {}):V {
        ...
        if (typeCopyers || presetTCMap.size > 0){
            let mergedTCArr = mergeTypeRevivers(presetTCMap,typeCopyers);
            const trObj = toTypeReviverObject(mergedTCArr);
            const copier:Copier = trObj[typeName];
            if (typeof copier === "function"){
                return  copier.call(value,value,copyMember,key,host,typeName,depth);
            }
        }


        if (typeof (<any>value).getCopy === "function"){
            return (<any>value).getCopy(value,copyMember,key,host,typeName,depth);
        }
